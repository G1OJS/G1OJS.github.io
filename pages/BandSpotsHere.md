...
## Notes
At the time of writing (Sep 2024), I've been working on this utility for about a week; hence, this page is really to publish the idea and allow people who know a bit about coding to try it out. As I develop it further, I'll add some install instructions and see what I can do to make that as simple as possible. In the meantime: download & install [Python](https://www.python.org/), open a command window and type "pip install" + each of the libraries mentioned below in turn, download the two bits of Python code from this page, and the two Windows batch files (bottom of this page) to run them, put all 4 files in a convenient folder, and run both batch files.
